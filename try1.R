...
cap3_tux1.3<-cap3_tux1.2%>%#transformacion min max 
...
cap3_cova_tux<-cov(cap3_tux1.3)#calculo de la matriz de covarianzas
...
cap3_pc_tux<-prcomp(cap3_tux1.3)#calculo de acp y biplot 
...
cap3_rotac_tux_graf<-cap3_rotac_tux1.2%>%#rotacion min max de nuevo
  mutate(x=(x-min(x))/(max(x)-min(x)),
...
cap3_pc_chtzn<-prcomp(cap3_chintzn1.3)#calculo de componentes
...
cap3_rotac_chntz_graf<-cap3_rotac_chntz1.1%>%#rotacion min max 
  mutate(x=(x-min(x))/(max(x)-min(x)),
         y=(y-min(y))/(max(y)-min(y)))
...
d<-as.vector(coef(cap3_modelo_chtzn_nppp1.2))#extraccion de coeficientes en vector
...
for(i in 1:368){#368 es por el numero de volcanes individuales en los tuxtlas
  ...
  L_tux[i]= (d[1]+# d representa el vector de coeficientes del modelo ajustado
               d[2]*x_tux[i]+#x_tux representa la variable x guardado anteriormente de tux
...
cap3_modelo_H0_nppp1.2<-ppm(cap3_modelo_H0,~(x+y+I(x^2)+I(y^2)))#seleccion del modelo 
...
for(i in 1:724){#724 representa el numero total de datos 
  ...
  L_H0[i]= (d[1]+#d representa el vector de coeficientes del modelo 
              d[2]*x_H0[i]+#x_h0 representa al eje de las x
              d[3]*y_H0[i]+#y_h0 representa al eje de las y 
              d[4]*(x_H0[i]^2)+
              d[5]*(y_H0[i]^2))*724
...
H1<-Verosimilitud_chtzn*Verosimilitud_tux #Se calcula la verosimilitud para H1

G<- -2*log(Verosimilitud_H0)-(-(2*log(H1)))# se calcula el estadistico de prueba
...
for (i in 1:1000) {#se consideran 1000 iteraciones 
  ...
  cap3_modelo_perm1.2<-ppm(cap3_modelo_perm,~(x+y+I(x^2)+I(y^2)))#se ajusta el modelo a perm1
  ...
  for(j in 1:356){# se construye el ciclo para el calculo de la verosimilitud de perm1
    ...
    L_perm1[j]= (d_perm1[1]+#d_perm1 representa el vector de coeficientes del modelo
                   d_perm1[2]*x_perm1[j]+#x_perm representa la variable x de perm1
                   d_perm1[3]*y_perm1[j]+#y_perm representa la variable de y de perm1
  ...
  cap3_modelo_perm2.22<-ppm(cap3_modelo_perm2,~(x+y+I(x^2)+I(y^2)))#se ajusta el modelo a perm2
  ...
  for(l in 1:368){#se inicia el ciclo para el calculo de la verosimilitud de perm2
    ...
    L_perm2[l]= (d_perm2[1]+#d se refiere a el vector de coeficientes del modelo ajustado de perm2
                   d_perm2[2]*x_perm2[l]+#x_perm2 representa la variable x de perm2
                   d_perm2[3]*y_perm2[l]+#y_perm2 representa la variable y de perm2
  ...
  H1_perm<-Verosimilitud_perm2*Verosimilitud_perm1#se calcula la verosimilitud de H1
  
  G_perm[i]<- -2*log(Verosimilitud_H0)-(-2*log(H1_perm))#se calcula la razon de verosimilitudes 
...
((sum(G_perm>=G))+1)/(1000+1)# p valor< 0.05
...
as.data.frame(G_perm)%>%
  ...
  #linetype="")+
  labs(x="Cálculo del Estadístico de Prueba",
       y="Densidad")+
