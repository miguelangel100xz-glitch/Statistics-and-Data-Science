...
pairs(x = datos[,c("Salario","T.lote","horas")],
      col = c("firebrick","green3")[datos$Grupos],pch = 19)

#install.packages("scatterplot3d")
library(scatterplot3d)

scatterplot3d(cesped$Salario, cesped$T.lote, cesped$horas,
              color = c("firebrick", "green3")[cesped$Grupos], pch = 19,
...
p1 <- ggplot(data = datos, aes(x = Salario)) + 
geom_histogram(position = "identity", 
               alpha = 0.5, 
               aes(fill = as.factor(Grupos)))+
labs(fill = "Grupos")

p2 <- ggplot(data = datos, aes(x = T.lote)) + 
geom_histogram(position = "identity", 
               alpha = 0.5, 
               aes(fill = as.factor(Grupos)))+
labs(fill = "Grupos")
...
for (k in 2:4) {
  ...
  for (i in 1:2) {
    i0 <- levels(datos$Grupos)[i]
    x <- datos[datos$Grupos == i0, j0]
...
kable(datos_tidy %>% group_by(Grupos, variable) %>% summarise(p_value_Shapiro.test = shapiro.test(valor)$p.value))
