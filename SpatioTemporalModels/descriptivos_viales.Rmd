...
```{r}
viales %>%
    filter(clas_con_f_alarma=="URGENCIAS MEDICAS")%>%
    filter(delegacion_inicio =="IZTAPALAPA")%>%
    count(incidente_c4)%>%
    mutate(prop=n/sum(n))%>%
    ggplot(aes(x=reorder(incidente_c4,prop),y=prop,fill=incidente_c4))+geom_col(alpha=.5)+coord_flip()+scale_y_continuous(labels = scales::percent)+guides(fill=FALSE)+labs(y="Porcentaje",x="Incidente")+theme_bw()

viales %>%
    filter(clas_con_f_alarma=="URGENCIAS MEDICAS")%>%
    filter(delegacion_inicio =="GUSTAVO A. MADERO")%>%
    count(incidente_c4)%>%
    mutate(prop=n/sum(n))%>%
    ggplot(aes(x=reorder(incidente_c4,prop),y=prop,fill=incidente_c4))+geom_col(alpha=.5)+coord_flip()+scale_y_continuous(labels = scales::percent)+guides(fill=FALSE)+labs(y="Porcentaje",x="Incidente")+theme_bw()
...
viales %>%
    filter(clas_con_f_alarma=="URGENCIAS MEDICAS")%>%
    filter(delegacion_inicio =="CUAUHTEMOC")%>%
    count(incidente_c4)%>%
    mutate(prop=n/sum(n))%>%
    ggplot(aes(x=reorder(incidente_c4,prop),y=prop,fill=incidente_c4))+geom_col(alpha=.5)+coord_flip()+scale_y_continuous(labels = scales::percent)+guides(fill=FALSE)+labs(y="Porcentaje",x="Incidente")+theme_bw()
